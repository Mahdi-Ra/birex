...
class GeneralSettingController extends Controller
{
    ...
    {
        $data['page_title'] = "All New Auth";
        $data['users'] = User::where('auth_flag', true)->paginate(20);
        return view('admin.users.index', $data);
    }

    public function users_new()
    {
        $data['page_title'] = "All New User";
        $data['users'] = User::whereDate('created_at', Carbon::today())->paginate(20);
        return view('admin.users.index', $data);
    }
    ...
    public function saveBalanceByUsers(Request $request)
    {
        ...
        if($request->operation == "on")
        {
            ...
            if($balance->balance > 0 && $balance->balance >= $request->amount)
            {
                $balance->balance -= $request->amount;
                if($balance->balance<0.000000000000000001)
                {
                    $balance->balance=0;
                }
                $balance->save();
